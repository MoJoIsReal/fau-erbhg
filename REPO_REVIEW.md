...
| PERF-003 | `/kalender.ics` is public, polled repeatedly by calendar clients, uncached (`/api/*` is `no-store`), two queries per hit | HIGH CONFIDENCE |
...
| `?lang=en` through the ICS rewrite | `api/*.js` cannot run locally, by design | English subscribers may silently receive the Norwegian feed — a soft failure nobody reports | `curl -s 'https://<preview>/kalender.ics?lang=en' \| head` |
