...
### Progress
- [x] Centralized JWT configuration rejects missing, short and placeholder secrets.
- [x] Signing and verification are bound to HS256, issuer and audience.
- [x] Added regression tests for weak configuration, legacy context-free tokens and wrong algorithms.
- [ ] Confirm the deployed secret is strong and notify users about the one-time session invalidation.
...
### Progress
- [x] Added one shared provider-error reporter for redacted console and Sentry reporting.
- [x] Migrated contact, acknowledgement, newsletter, registration-reminder and reply error paths.
- [x] Added synthetic email/phone redaction tests.
...
### Progress
- [x] Enforced MIME/extension compatibility and safe non-negative integer sizes.
- [x] Bound Cloudinary resource type and image format to the validated upload metadata.
- [x] Persist provider-observed size and added mismatch/boundary regression tests.
- [ ] Verify every allowed format against the production Cloudinary account in staging.
...
### Progress
- [x] Server now ignores client attribution and persists the authenticated username.
- [x] Removed the misleading uploader selector and client-supplied attribution fields.
- [ ] Add an authenticated upload handler integration test when the provider harness is available.
