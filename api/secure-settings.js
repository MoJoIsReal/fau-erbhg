...
function mapBlogPost(row) {
  ...
  return {
    ...
    author: row.author,
    // NULL means not on the homepage, everywhere: here, in the editor list and
    // in the homepage query below (IS TRUE).
...
function withoutTimestamps({ createdAt, updatedAt, ...rest }) {
  return rest;
}

// What anyone may read: the post, not its editorial state or who wrote it in.
function publicBlogPost({ status, notifyNewsletter, newsletterSentAt, createdBy, createdAt, updatedAt, ...post }) {
  return post;
}
...
// The tokens are never selected: this is an admin list, not a way to act as
// the subscriber.
function mapNewsletterSubscriber(row) {
  ...
  return {
    ...
    // Outbox rows still to go out (queued, or waiting to be retried) and
    // rows given up on after the cron's last attempt. Both age out with the
    // 90-day delivery retention.
...
async function handleBoardMembers(req, res, sql) {
  ...
  if (req.method === 'GET') {
    ...
    // The public list has never carried the timestamps.
    const members = await sql`
      ...
      ORDER BY sort_order ASC,
               ...
               id ASC
    `;
    return res.status(200).json(members.map(mapBoardMember).map(withoutTimestamps));
  }
...
async function handleBlogPosts(req, res, sql) {
  ...
  if (req.method === 'GET') {
    ...
    // limit/offset are opt-in: omitting them returns up to 500 posts in one
    // page. Callers that do "load more" pagination (the public news page) pass
    // both explicitly. A value that is present but not an integer in range is
    // refused rather than quietly replaced: `limit=1.5` used to reach LIMIT
    // and come back as a 500.
    const limit = req.query.limit != null ? sanitizeInteger(req.query.limit, 1, 100) : 500;
    const offset = req.query.offset != null ? sanitizeInteger(req.query.offset, 0) : 0;
    if (limit === null || offset === null) {
      return res.status(400).json({ error: 'limit must be a whole number from 1 to 100, offset a whole number from 0' });
    }
    ...
    const postId = req.query.id != null ? sanitizeInteger(req.query.id, 1, MAX_INT_ID) : null;
    if (req.query.id != null && !postId) {
      return res.status(400).json({ error: 'Valid id query parameter required' });
    }
    // The homepage shows three posts; it used to download up to 500 and filter
    // them itself. IS TRUE, so a NULL flag is not shown, as mapBlogPost says.
    ...
    if (includeArchived === 'true') {
      ...
      `;
      return res.status(200).json(posts.map(mapBlogPost));
    }

    // Public view - only show published posts
    const posts = await sql`
      SELECT id, title, content, category, published_date, author, show_on_homepage
      FROM blog_posts
      WHERE status = 'published'
        AND (${postId}::int IS NULL OR id = ${postId})
        AND (${sanitizedCategory}::text IS NULL OR category = ${sanitizedCategory})
        AND (${homepageOnly} = false OR show_on_homepage IS TRUE)
        AND (${searchPattern}::text IS NULL OR title ILIKE ${searchPattern}
          OR regexp_replace(content, '<[^>]*>', ' ', 'g') ILIKE ${searchPattern})
      ORDER BY published_date DESC
      LIMIT ${limit} OFFSET ${offset}
    `;
    return res.status(200).json(posts.map(mapBlogPost).map(publicBlogPost));
  }
...
async function handleKindergartenInfo(req, res, sql) {
  ...
  if (req.method === 'PUT') {
    ...
    const sanitizedNumberOfChildren = sanitizeInteger(numberOfChildren, 0, 1000);
...
async function handleContactMessages(req, res, sql) {
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json(messages.map(mapContactMessage));
  }
  ...
  if (req.method === 'POST') {
    ...
    return res.status(200).json(mapContactMessage(updated[0]));
  }
...
async function handleUsers(req, res, sql) {
  ...
  if (req.method === 'POST') {
    ...
    // the race it cannot see. Two admins creating the same user at once used
    // to both pass the lookup, and the second INSERT was a unique violation —
...
async function handleNewsletterSubscribers(req, res, sql) {
  ...
  if (req.method === 'GET') {
    // With each subscriber's outbox counts, so a failing address is visible
    // to the council on the settings page rather than only in the logs.
    const subscribers = await sql`
      SELECT s.id, s.email, s.name, s.language, s.status,
             s.created_at, s.confirmed_at, s.unsubscribed_at,
             COUNT(d.id) FILTER (WHERE d.status IN ('pending', 'processing'))::int AS pending_deliveries,
             COUNT(d.id) FILTER (WHERE d.status = 'failed')::int AS failed_deliveries
      FROM newsletter_subscribers s
      LEFT JOIN newsletter_deliveries d ON d.subscriber_id = s.id
      GROUP BY s.id
      ORDER BY s.created_at DESC
    `;
    return res.status(200).json(subscribers.map(mapNewsletterSubscriber));
  }
